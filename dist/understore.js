(function(global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() : typeof define === 'function' && define.amd ? define(factory) : (global.understore = factory());
}(this, (function() {
	var understore = {};
	var $tore;
	var $dom, dom = {};
	...
	function Catch(task) {
		...
		try {
			if (typeof task == "function") {
				...
				if (Chain.tasks.length == 1) Chain.task = !Chain.task ? setInterval(Chain, Chain.duration) : undefined;
				return {
					then: Then,
					fetch: Fetch,
					catch: Catch
	...
	function Template(html, obj, id, idx) {
		...
		var add = function(line, js) {
			...
			var _code = '\ var s = r[r.length-1];\ var key = s.match(' + attrRe + ');\ var $key = "' + key + '";\ if(s[s.length-1] == ">") { \ r[r.length-1] = s.replace(/>$/," _' + key + '>")\ }else if(key){\ key = key[key.length-1];\ var _key = key.replace("=\\\"", "").replace("=\\\'", "");\ _key = _key != $key ? "-"+$key+"=\\\""+_key+"\\\"" : "-"+_key;\ r[r.length-1] = s.replace(key, ""+_key+" "+key);\ };\ r.push(' + line + ');\n\n';
	...
	function SetItem(option) {
		...
		if (oldValue != _newValue) {
			typeof newValue.$tate == "undefined" ? newValue.$tate = {
				type: "set"
			} : newValue.$tate.type = "set";
